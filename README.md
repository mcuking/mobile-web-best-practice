...
**1.子项目的路由一定是要提前加载到主项目的总的路由表里吗？**

不一定要提前加载路由。也可以做成懒加载子项目的路由，也就是说等到用户点击要进入子项目模块，通过分析即将跳转的路由确定是哪一个子项目，然后再异步去加载该子项目的入口文件 main.js（可以通过 [systemjs](https://github.com/systemjs/systemjs) 或者自己写一个动态创建 script 标签并插入 body 的方法）。加载成功后就可以将子项目的路由动态添加到主项目总的路由里了。

这里也给大家推荐一个懒加载路由的 demo：

[async-routes](https://github.com/micro-frontends-vue/async-routes)

下面是主项目 router.js 文件中异步加载子项目入口文件的核心代码：
...
**2.如果子项目代码更新后，除了打包部署子项目之外，还需要打包部署主项目吗？**
